...
- **Mini Prompt Gráfico (`! _is_raw_tty`):** Em emuladores modernos, suporta dois layouts de alta densidade via `$PROMPT_STYLE`:
  - **Pílula (`pill` — padrão em FreeBSD 14+):** Exibe pílulas com versão (` 15.1`), pasta (`❮ shell❯`), usuário (`❮ gabrielf❯`) e Git (`❮󰊢 main*❯ `), calibrado estritamente entre 177 e 188 bytes no buffer de 192 bytes.
  - **Micro (`micro` — auto-fallback em FreeBSD <= 13 ou `$PROMPT_BUFFER_LIMIT < 192`):** Layout _streamlined_ ultra-leve com ícones diretos sem delimitadores redundantes (` 13.2  shell  gabrielf 󰊢 main* `), calibrado entre 96 e 126 bytes no buffer estático de 128 bytes.
...
- **Fallback TTY 100% Bold (`_is_raw_tty`):** No console puro `vt`/`syscons`, comuta para prompt ASCII atômico de linha única 100% negrito via persistência ANSI ECMA-48, com orçamento dinâmico operando entre 120 e 190 bytes com salvaguarda absoluta contra overflow.
