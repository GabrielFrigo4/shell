...
- **Mini Prompt Gráfico (`! _is_raw_tty`):** Em emuladores modernos, suporta dois layouts de alta densidade via `$PROMPT_STYLE`:
  - **Pílula (`pill` — padrão em FreeBSD 14+):** Exibe pílulas com versão (` 15.1`), pasta (`❮ shell❯`), usuário (`❮ gabrielf❯`) e Git (`❮󰊢 main*❯ `), calibrado estritamente entre 170 e 185 bytes no buffer de 192 bytes.
