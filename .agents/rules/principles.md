...
- **`kebab-case` (público):** Comandos destinados ao uso interativo (`mount-device`, `update-all`, `open-helix`, `path-front`). Definidos diretamente, sem wrappers redundantes.
...
## 4. Estrutura de Arquivos e Seções
- Todo arquivo deve ter cabeçalho de módulo no padrão:
  ```sh
  ### ================================
...
- Seções internas usam divisores:
  ```sh
  ### --------------------------------
  ### Nome da Secao
  ### --------------------------------
...
2. `find . -name "*.sh" -exec sh -n {} +` (100% livre de erros).
3. `find . -name "*.sh" -exec bash -n {} +` (100% livre de erros).
