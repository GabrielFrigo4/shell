...
_theme_render() {
	local _delimiter="${_theme_color_delimiter}"
	local _git_info=""
	if [ -n "${_branch}" ]; then
		local _indicator=""
...
"
	...
	PS1="${PS1}${_newline}${_delimiter}┌──❮${_theme_color_blue} ${_theme_color_green}${_time}${_delimiter}❯─❮${_theme_color_blue} ${_theme_color_green}${_date}${_delimiter}❯─❮${_theme_color_yellow} ${_theme_color_blue}${_pwd}${_delimiter}❯─ ❮${_user_icon_color:-${_theme_color_blue}} ${_user_color}${_user}${_delimiter}❯${_git_info}"
