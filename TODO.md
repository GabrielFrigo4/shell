...
- [ ] 🖥️ **1. Adicionar Suporte 100% para WSL2** 
  - Criar um novo contexto isolado (`--context wsl`), pois o WSL2 possui comportamentos únicos e não se enquadra perfeitamente como *Server*, *Desktop* ou *Container*.
...
- [ ] 🔄 **4. Criar função "Super Upsh" (Atualização Profunda)** 
  - Criar o comando `insh` (abreviação de *install shell*) que vá além de apenas um `git pull` (que é o que o `upsh` atual faz).
