...
if [ "${OS}" != "windows" ]; then
	...
	if sudo grep -qF "${SOURCE_LINE}" "${ROOT_RC_FILE}" 2> "/dev/null"; then
		...
		echo "# === Shell repo config (auto-generated by install.sh) ===" | sudo tee -a "${ROOT_RC_FILE}" > "/dev/null"
		echo "${CONTEXT_LINE}" | sudo tee -a "${ROOT_RC_FILE}" > "/dev/null"
		echo "${CORE_ENV_LINE}" | sudo tee -a "${ROOT_RC_FILE}" > "/dev/null"
